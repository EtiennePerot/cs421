...
(
	SELECT
		'audio' as `type`,
		title,
		ROUND(`SUM(songs.duration)` / 60) AS totalDuration
	FROM items NATURAL JOIN (
		SELECT audio.iid, SUM(songs.duration) FROM audio NATURAL JOIN songs
		GROUP BY audio.iid
...
SELECT pnid, balance FROM members WHERE standing = 'bad' AND EXISTS (
